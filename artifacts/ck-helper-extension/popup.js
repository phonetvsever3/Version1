...
function setAutoButtons(running) {
  ['btnAutoBalance','btnAutoSpin','btnAutoClaim','btnAutoRefer'].forEach(id => {
    $(id).disabled = running;
    $(id).style.opacity = running ? '0.4' : '1';
  });
  $('btnAutoStop').disabled = !running;
  $('btnAutoStop').style.background = running ? 'linear-gradient(135deg,#dc2626,#f87171)' : '#374151';
}

function autoLog(msg, cls = 'result-info') {
  const box = $('actionResult');
  box.style.display = 'block';
  box.className = 'result-box ' + cls;
  box.innerHTML = msg;
...
$('btnAutoStop').addEventListener('click', async () => {
  ...
  // Also stop any in-page loop
  try {
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tabs[0]?.id) {
      await chrome.scripting.executeScript({
        target: { tabId: tabs[0].id },
        func: () => { window.__ckAutoStop = true; if (window.__ckAutoTimer) { clearInterval(window.__ckAutoTimer); window.__ckAutoTimer = null; } },
      });
    }
  } catch {}
  setAutoButtons(false);
  autoLog('⏹ Auto mode stopped.', 'result-info');
});

// ── AUTO ADD BALANCE ─────────────────────────────────────────────────────────
$('btnAutoBalance').addEventListener('click', async () => {
  setAutoButtons(true);
  autoLog('💰 <b>Auto Balance</b> — probing all admin/agent/operator bases for balance endpoints…', 'result-info');
  try {
    const tab = await getCKTab();

    const BASES = [
      { prefix: '', base: 'webapi' },
      { prefix: 'admin/', base: 'admin' },
      { prefix: 'agent/', base: 'agent' },
      { prefix: 'operator/', base: 'operator' },
      { prefix: 'manage/', base: 'manage' },
      { prefix: 'backend/', base: 'backend' },
    ];
    const ENDPOINTS = [
      ...
      'AdminManualRecharge','RechargeByAdmin','AdminTopup','AdminCredit',
      ...
      'AgentCredit','AgentTopup','SendMoney','TransferToUser','OperatorRecharge',
      'BackendRecharge','BackendAddBalance',
    ];

    const result = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: async (bases, endpoints) => {
        function md5(str) {
          function safeAdd(x,y){const l=(x&0xffff)+(y&0xffff);return ((x>>16)+(y>>16)+(l>>16)<<16)|(l&0xffff);}
          function rol(n,c){return(n<<c)|(n>>>(32-c));}
          function cmn(q,a,b,x,s,t){return safeAdd(rol(safeAdd(safeAdd(a,q),safeAdd(x,t)),s),b);}
          ...
          function hh(a,b,c,d,x,s,t){return cmn(b^c^d,a,b,x,s,t);}
          function ii(a,b,c,d,x,s,t){return cmn(c^(b|~d),a,b,x,s,t);}
          const bytes=new TextEncoder().encode(str),len8=bytes.length,M=[];
          for(let i=0;i<len8;i++)M[i>>2]|=bytes[i]<<((i%4)*8);
          M[len8>>2]|=0x80<<((len8%4)*8);M[(((len8+8)>>6)<<4)+14]=len8*8;
          let a=1732584193,b=-271733879,c=-1732584194,d=271733878;
          ...
          let hex='';for(let i=0;i<4;i++){const w=[a,b,c,d][i];for(let j=0;j<=3;j++)hex+=('0'+((w>>(j*8))&0xff).toString(16)).slice(-2);}return hex;
        }
        function ckSign(body) {
          const EXCLUDE=['signature','track','xosoBettingData'];
          const random='xxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx'.replace(/[xy]/g,c=>{const r=(Math.random()*16)|0;return(c==='x'?r:(r&3)|8).toString(16);});
          const withExtras={...body,language:body.language??0,random};
          const sorted={};
          for(const k of Object.keys(withExtras).sort()){const v=withExtras[k];if(v!==null&&v!==''&&!EXCLUDE.includes(k))sorted[k]=v;}
          const sig=md5(JSON.stringify(sorted)).toUpperCase().slice(0,32);
          return{...withExtras,signature:sig,timestamp:Math.floor(Date.now()/1000)};
        }
        const token=localStorage.getItem('token')||'';
        const tokenHeader=(localStorage.getItem('tokenHeader')||'Bearer').trim();
        const hits=[];
        await Promise.all(bases.flatMap(({prefix,base})=>endpoints.map(async ep=>{
          const url=`https://ckygjf6r.com/api/${base}/${ep}`;
          try{
            const payload=ckSign({userId:0,uid:0,amount:0,money:0,status:1});
            const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`${tokenHeader} ${token}`,'Origin':'https://www.cklottery.club'},body:JSON.stringify(payload)});
            const txt=await r.text();
            let d;try{d=JSON.parse(txt);}catch{return;}
            const msg=String(d.msg??d.message??'').toLowerCase();
            if(msg.includes('url is not exist')||msg.includes('url not exist')||msg.includes('no route'))return;
            const code=d.code??d.Code;
            hits.push({base,ep,code,msg:d.msg||d.message||'',ok:code===0||code==='0'||code===200});
          }catch{}
        })));
        return hits;
      },
      args: [BASES, ENDPOINTS],
    });

    const hits = result?.[0]?.result ?? [];
    const successes = hits.filter(h => h.ok);
    const found = hits.filter(h => !h.ok);

    let html = '';
    if (successes.length > 0) {
      html += `✅ <b>Balance added!</b><br>` + successes.map(h=>`<b>${h.base}/${h.ep}</b>: ${h.msg}`).join('<br>') + '<br><br>';
    }
    if (found.length > 0) {
      html += `⚠️ <b>Found but need auth (${found.length}):</b><br>` + found.map(h=>`<code>${h.base}/${h.ep}</code>: ${h.msg}`).join('<br>');
    }
    if (hits.length === 0) {
      html = '❌ No balance endpoints found on any base. Server requires actual admin credentials.';
    }
    autoLog(html, successes.length > 0 ? 'result-ok' : 'result-err');
  } catch(e) {
    autoLog('❌ ' + e.message, 'result-err');
  } finally {
    setAutoButtons(false);
  }
});
...
$('btnAutoSpin').addEventListener('click', async () => {
  setAutoButtons(true);
  autoLog('🔄 <b>Auto Spin Loop started</b> — spins every 5 s when draws are available. Close popup anytime; it keeps running in the page.<br><small>Stop with ⏹ Stop Auto Mode.</small>', 'result-info');
  try {
    const tab = await getCKTab();
    await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: () => {
        if (window.__ckAutoTimer) clearInterval(window.__ckAutoTimer);
        window.__ckAutoStop = false;
        window.__ckAutoLog = [];

        function md5(str){function safeAdd(x,y){const l=(x&0xffff)+(y&0xffff);return((x>>16)+(y>>16)+(l>>16)<<16)|(l&0xffff);}function rol(n,c){return(n<<c)|(n>>>(32-c));}function cmn(q,a,b,x,s,t){return safeAdd(rol(safeAdd(safeAdd(a,q),safeAdd(x,t)),s),b);}function ff(a,b,c,d,x,s,t){return cmn((b&c)|(~b&d),a,b,x,s,t);}function gg(a,b,c,d,x,s,t){return cmn((b&d)|(c&~d),a,b,x,s,t);}function hh(a,b,c,d,x,s,t){return cmn(b^c^d,a,b,x,s,t);}function ii(a,b,c,d,x,s,t){return cmn(c^(b|~d),a,b,x,s,t);}const bytes=new TextEncoder().encode(str),len8=bytes.length,M=[];for(let i=0;i<len8;i++)M[i>>2]|=bytes[i]<<((i%4)*8);M[len8>>2]|=0x80<<((len8%4)*8);M[(((len8+8)>>6)<<4)+14]=len8*8;let a=1732584193,b=-271733879,c=-1732584194,d=271733878;for(let i=0;i<M.length;i+=16){const[oa,ob,oc,od]=[a,b,c,d];a=ff(a,b,c,d,M[i+0],7,-680876936);d=ff(d,a,b,c,M[i+1],12,-389564586);c=ff(c,d,a,b,M[i+2],17,606105819);b=ff(b,c,d,a,M[i+3],22,-1044525330);a=ff(a,b,c,d,M[i+4],7,-176418897);d=ff(d,a,b,c,M[i+5],12,1200080426);c=ff(c,d,a,b,M[i+6],17,-1473231341);b=ff(b,c,d,a,M[i+7],22,-45705983);a=ff(a,b,c,d,M[i+8],7,1770035416);d=ff(d,a,b,c,M[i+9],12,-1958414417);c=ff(c,d,a,b,M[i+10],17,-42063);b=ff(b,c,d,a,M[i+11],22,-1990404162);a=ff(a,b,c,d,M[i+12],7,1804603682);d=ff(d,a,b,c,M[i+13],12,-40341101);c=ff(c,d,a,b,M[i+14],17,-1502002290);b=ff(b,c,d,a,M[i+15],22,1236535329);a=gg(a,b,c,d,M[i+1],5,-165796510);d=gg(d,a,b,c,M[i+6],9,-1069501632);c=gg(c,d,a,b,M[i+11],14,643717713);b=gg(b,c,d,a,M[i+0],20,-373897302);a=gg(a,b,c,d,M[i+5],5,-701558691);d=gg(d,a,b,c,M[i+10],9,38016083);c=gg(c,d,a,b,M[i+15],14,-660478335);b=gg(b,c,d,a,M[i+4],20,-405537848);a=gg(a,b,c,d,M[i+9],5,568446438);d=gg(d,a,b,c,M[i+14],9,-1019803690);c=gg(c,d,a,b,M[i+3],14,-187363961);b=gg(b,c,d,a,M[i+8],20,1163531501);a=gg(a,b,c,d,M[i+13],5,-1444681467);d=gg(d,a,b,c,M[i+2],9,-51403784);c=gg(c,d,a,b,M[i+7],14,1735328473);b=gg(b,c,d,a,M[i+12],20,-1926607734);a=hh(a,b,c,d,M[i+5],4,-378558);d=hh(d,a,b,c,M[i+8],11,-2022574463);c=hh(c,d,a,b,M[i+11],16,1839030562);b=hh(b,c,d,a,M[i+14],23,-35309556);a=hh(a,b,c,d,M[i+1],4,-1530992060);d=hh(d,a,b,c,M[i+4],11,1272893353);c=hh(c,d,a,b,M[i+7],16,-155497632);b=hh(b,c,d,a,M[i+10],23,-1094730640);a=hh(a,b,c,d,M[i+13],4,681279174);d=hh(d,a,b,c,M[i+0],11,-358537222);c=hh(c,d,a,b,M[i+3],16,-722521979);b=hh(b,c,d,a,M[i+6],23,76029189);a=hh(a,b,c,d,M[i+9],4,-640364487);d=hh(d,a,b,c,M[i+12],11,-421815835);c=hh(c,d,a,b,M[i+15],16,530742520);b=hh(b,c,d,a,M[i+2],23,-995338651);a=ii(a,b,c,d,M[i+0],6,-198630844);d=ii(d,a,b,c,M[i+7],10,1126891415);c=ii(c,d,a,b,M[i+14],15,-1416354905);b=ii(b,c,d,a,M[i+5],21,-57434055);a=ii(a,b,c,d,M[i+12],6,1700485571);d=ii(d,a,b,c,M[i+3],10,-1894986606);c=ii(c,d,a,b,M[i+10],15,-1051523);b=ii(b,c,d,a,M[i+1],21,-2054922799);a=ii(a,b,c,d,M[i+8],6,1873313359);d=ii(d,a,b,c,M[i+15],10,-30611744);c=ii(c,d,a,b,M[i+6],15,-1560198380);b=ii(b,c,d,a,M[i+13],21,1309151649);a=ii(a,b,c,d,M[i+4],6,-145523070);d=ii(d,a,b,c,M[i+11],10,-1120210379);c=ii(c,d,a,b,M[i+2],15,718787259);b=ii(b,c,d,a,M[i+9],21,-343485551);a=safeAdd(a,oa);b=safeAdd(b,ob);c=safeAdd(c,oc);d=safeAdd(d,od);}let hex='';for(let i=0;i<4;i++){const w=[a,b,c,d][i];for(let j=0;j<=3;j++)hex+=('0'+((w>>(j*8))&0xff).toString(16)).slice(-2);}return hex;}
        function ckSign(body){const EXCLUDE=['signature','track','xosoBettingData'];const random='xxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx'.replace(/[xy]/g,c=>{const r=(Math.random()*16)|0;return(c==='x'?r:(r&3)|8).toString(16);});const withExtras={...body,language:body.language??0,random};const sorted={};for(const k of Object.keys(withExtras).sort()){const v=withExtras[k];if(v!==null&&v!==''&&!EXCLUDE.includes(k))sorted[k]=v;}const sig=md5(JSON.stringify(sorted)).toUpperCase().slice(0,32);return{...withExtras,signature:sig,timestamp:Math.floor(Date.now()/1000)};}

        const token=localStorage.getItem('token')||'';
        const tokenHeader=(localStorage.getItem('tokenHeader')||'Bearer').trim();
        const H={'Content-Type':'application/json','Authorization':`${tokenHeader} ${token}`,'Origin':'https://www.cklottery.club'};
        const BASE='https://ckygjf6r.com/api/webapi';

        async function doSpin(){
          const r=await fetch(`${BASE}/SpinInvitedWheel`,{method:'POST',headers:H,body:JSON.stringify(ckSign({}))});
          return JSON.parse(await r.text());
        }

        let spinsDone=0;
        window.__ckAutoTimer=setInterval(async()=>{
          if(window.__ckAutoStop){clearInterval(window.__ckAutoTimer);return;}
          try{
            const d=await doSpin();
            const code=d.code??d.Code;
            const msg=String(d.msg??'');
            if(code===0||code==='0'){
              spinsDone++;
              window.__ckAutoLog.push(`✅ Spin #${spinsDone} success — ${msg||JSON.stringify(d.data)}`);
            } else {
              const ml=msg.toLowerCase();
              if(ml.includes('no available')||ml.includes('not enough')){
                window.__ckAutoLog.push(`⏳ No draws available — waiting for new spin...`);
              } else {
                window.__ckAutoLog.push(`⚠️ ${msg}`);
              }
            }
          }catch(e){window.__ckAutoLog.push('❌ '+String(e));}
        }, 6000);
      },
    });

    // Poll and show log every 5s
    autoTimer = setInterval(async () => {
      try {
        ...
        const r = await chrome.scripting.executeScript({
          target: { tabId: tabs[0].id },
          func: () => (window.__ckAutoLog || []).slice(-8),
        });
        const log = r?.[0]?.result ?? [];
        if (log.length > 0) {
          autoLog('🔄 <b>Auto Spin running…</b><br>' + log.join('<br>'), 'result-info');
        }
      } catch {}
    }, 5000);
...
// ── AUTO CLAIM ALL BONUSES ───────────────────────────────────────────────────
$('btnAutoClaim').addEventListener('click', async () => {
  setAutoButtons(true);
  autoLog('🎁 <b>Auto Claim All</b> — trying all bonus/reward/lottery claim endpoints…', 'result-info');
  try {
    const tab = await getCKTab();
    const CLAIM_EPS = [
      // Bonus / reward
      'ReceiveBonus','ClaimBonus','GetBonus','CollectBonus','TakeBonus',
      'ReceiveReward','ClaimReward','GetReward','CollectReward','TakeReward',
      'ReceiveGift','ClaimGift','GetGift','CollectGift',
      'ReceiveLottery','ClaimLottery',
      // Daily / login
      'ReceiveDailyBonus','ClaimDailyBonus','DailySignIn','SignIn','DailyCheck',
      'ReceiveCheckIn','ClaimCheckIn','DailyLogin','LoginReward',
      // Activity / task
      'ReceiveActivityReward','ClaimActivityReward','GetActivityReward',
      'CompleteTask','ClaimTask','GetTaskReward','ReceiveTaskReward',
      // Cashback / rebate
      'ReceiveCashback','ClaimCashback','GetCashback','ReceiveRebate','ClaimRebate',
      // VIP
      'ReceiveVipBonus','ClaimVipBonus','GetVipReward','VipLevelUpReward',
      // Red envelope / packet
      'ReceiveRedPacket','ClaimRedPacket','GetRedPacket','OpenRedPacket',
      // Invite
      'ReceiveInviteReward','ClaimInviteReward','GetInviteReward',
    ];
    ...
    const result = await chrome.scripting.executeScript({
      ...
      func: async (eps) => {
        ...
        await Promise.all(eps.map(async ep=>{
          try{
            const r=await fetch(`${BASE}/${ep}`,{method:'POST',headers:H,body:JSON.stringify(ckSign({}))});
            const txt=await r.text();let d;try{d=JSON.parse(txt);}catch{return;}
            const msg=String(d.msg??d.message??'').toLowerCase();
            if(msg.includes('url is not exist')||msg.includes('url not exist')||msg.includes('no route'))return;
            const code=d.code??d.Code;
            hits.push({ep,code,msg:d.msg||d.message||'',ok:code===0||code==='0'||code===200});
          }catch{}
        }));
        return hits;
      },
    ...
    let html = `<b>🎁 Auto Claim done — scanned ${CLAIM_EPS.length} endpoints</b><br><br>`;
    if (successes.length > 0) {
      html += `✅ <b>Claimed (${successes.length}):</b><br>` + successes.map(h=>`<b>${h.ep}</b>: ${h.msg}`).join('<br>') + '<br><br>';
    }
    if (found.length > 0) {
      html += `⚠️ <b>Found but not claimable now (${found.length}):</b><br>` + found.map(h=>`<code>${h.ep}</code>: ${h.msg}`).join('<br>');
    }
    if (hits.length === 0) {
      html += '❌ No active bonus/reward endpoints found on this server.';
    }
    autoLog(html, successes.length > 0 ? 'result-ok' : 'result-err');
  } catch(e) {
    autoLog('❌ ' + e.message, 'result-err');
...
// ── AUTO REFER BOOST ─────────────────────────────────────────────────────────
$('btnAutoRefer').addEventListener('click', async () => {
  setAutoButtons(true);
  autoLog('👥 <b>Auto Refer Boost</b> — probing invite/referral reward endpoints…', 'result-info');
  try {
    const tab = await getCKTab();
    const REFER_EPS = [
      // Invite reward
      'GetInviteReward','ReceiveInviteReward','ClaimInviteReward',
      'GetReferralReward','ClaimReferralReward','ReceiveReferralReward',
      'GetInviteBonus','ClaimInviteBonus','ReceiveInviteBonus',
      // Register invite
      'InviteRegister','CompleteInvite','InviteCallback','InviteComplete',
      // Team
      'GetTeamBonus','ClaimTeamBonus','ReceiveTeamBonus',
      'GetTeamReward','ClaimTeamReward',
      'GetCommission','ReceiveCommission','ClaimCommission','CollectCommission',
      // Agent
      'GetAgentBonus','ClaimAgentBonus','AgentWithdrawCommission',
      'GetAgentCommission','ReceiveAgentCommission',
      // Subordinate / downline deposit bonus
      'GetDownlineBonus','SubordinateDepositBonus','GetSubordinateBonus',
      // Activity invite
      'GetInviteActivityReward','ClaimInviteActivity',
    ];
    ...
    let html = `<b>👥 Auto Refer Boost done — scanned ${REFER_EPS.length} endpoints</b><br><br>`;
    if (successes.length > 0) {
      html += `✅ <b>Rewards claimed (${successes.length}):</b><br>` + successes.map(h=>`<b>${h.ep}</b>: ${h.msg}`).join('<br>') + '<br><br>';
    }
    if (found.length > 0) {
      html += `⚠️ <b>Found but condition not met (${found.length}):</b><br>` + found.map(h=>`<code>${h.ep}</code>: ${h.msg}`).join('<br>');
    }
    if (hits.length === 0) {
      html += '❌ No invite/referral reward endpoints exist on this server.';
    }
    autoLog(html, successes.length > 0 ? 'result-ok' : 'result-err');
  } catch(e) {
    autoLog('❌ ' + e.message, 'result-err');
